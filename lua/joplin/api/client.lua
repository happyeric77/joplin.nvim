...
local function execute_delete_request(cmd, retry_count)
  ...
  for attempt = 1, retry_count do
    ...
    if vim.v.shell_error == 0 then
      ...
      last_error =
        string.format("HTTP DELETE request failed (attempt %d/%d): %s", attempt, retry_count, result or "Unknown error")
...
local function execute_request(cmd, retry_count)
  ...
  for attempt = 1, retry_count do
    ...
    if vim.v.shell_error == 0 then
      ...
      last_error =
        string.format("HTTP request failed (attempt %d/%d): %s", attempt, retry_count, result or "Unknown error")
